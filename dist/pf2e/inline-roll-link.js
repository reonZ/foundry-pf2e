...
const InlineRollLinks = {
    injectRepostElement: (links, foundryDoc) => {
        for (const link of links) {
            ...
            const icon = link.parentElement?.dataset?.pf2Checkgroup !== undefined
                ? "fa-comment-alt-dots"
                : "fa-comment-alt";
    ...
    _onClickInlineAction: (event, link) => {
    ...
    _onClickInlineCheck: async (event, link, foundryDoc) => {
        ...
        const actors = (() => {
            ...
            const sheetActor = (() => {
                const maybeActor = foundryDoc instanceof ActorPF2e
                    ? foundryDoc
                ...
                for (const actor of actors) {
                    const statistic = (() => {
                        if (pf2Check in CONFIG.PF2E.magicTraditions && actor.isOfType("creature")) {
                            const bestSpellcasting = actor.spellcasting
                    ...
                    const item = (() => {
                        const itemFromDoc = foundryDoc instanceof ItemPF2e
                            ? foundryDoc
    ...
    _onClickInlineTemplate: (_event, html, link, foundryDoc) => {
        if (!canvas.ready)
            return;
        ...
        const messageId = foundryDoc instanceof ChatMessagePF2e
            ? foundryDoc.id
    ...
    repostAction: async (target, foundryDoc = null) => {
        ...
        const flags = foundryDoc instanceof JournalEntry
...
exports.InlineRollLinks = InlineRollLinks;
...
function resolveActor(foundryDoc, anchor) {
    if (foundryDoc instanceof ActorPF2e)
        return foundryDoc;
    if (foundryDoc instanceof ItemPF2e || foundryDoc instanceof ChatMessagePF2e)
