...
declare class Dialog extends Application {
     ...
     */
    static prompt(config?: DialogData & { label: string; rejectClose?: boolean }): Promise<unknown>;
     ...
     */
    static wait<R extends unknown>(
        data?: DialogWaitData<R>,
...
interface DialogWaitData<R extends unknown> extends Omit<DialogData, "buttons"> {
    buttons: Record<string, DialogButton<R>>;
}
...
interface DialogButton<R extends unknown = void> {
    ...
    label?: string;
    condition?: boolean;
    callback?: (html: JQuery) => R | Promise<R>;
}
